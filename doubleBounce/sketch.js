// ============================================================
//  Bounce Symphony — balls bouncing between N eroding concentric circles
...
// ============================
class ArcBoundary {
  constructor(cx, cy, radius, segments, collisionSide, hue) {
    this.cx = cx;
    this.cy = cy;
    this.r = radius;
  ...
  display() {
    ...
    {
      // Colour flows from cool blue (outer) to warm amber (inner) via hue.
...
function resetScene() {
  ...
  for (let i = 0; i < n; i++) {
    ...
    boundaries.push(new ArcBoundary(cx, cy, radius, ARC_SEGMENTS, side, hue));
  }
...
function draw() {
  ...
  if (!audioReady) {
    ...
    textSize(16);
    fill(150, 150, 170);
    text(`R to reset  ·  balls bounce between ${NUM_CIRCLES} eroding circle${NUM_CIRCLES > 1 ? 's' : ''}`, width / 2, height / 2 + 25);
    pop();
  }
